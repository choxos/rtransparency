...
detector's validated sensitivity and specificity (the Rogan-Gladen estimator).
...
Novelty and replication have no validated accuracy here, so their corrected
values are `NA`. To use your own validation (or the published `oddpub` values
for data and code), pass any table with `variable`, `sensitivity` and
`specificity` columns:
